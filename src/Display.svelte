...
<script>
	...
	onMount(() => {
		...
		let vertexShaderText = `
            ...
            void main() {
                gl_PointSize = ` + floatScale +`;
                //convert to display scale
                vec2 a_position2 = a_position * ` + floatScale +`;
		...
		let fragmentShader = createShader( gl, fragmentShaderText, gl.FRAGMENT_SHADER );
	...
	export function generateNoise(){
		for (let x = 0; x < width; x++){
			for (let y = 0; y < height; y++){
				setPixel(x, y, colorRGB(Math.random() * 255, Math.random() * 255, Math.random() * 255));
			}
		}
	...
	export function gradientFill(){
		for (let x = 0; x < width; x++){
			for (let y = 0; y < height; y++){
				setPixel(x, y, colorRGB(Math.floor(255 / height * y), Math.floor(255 / width * x), 0));
			}
		}
